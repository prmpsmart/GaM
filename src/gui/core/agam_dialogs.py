...
class PersonDialog(PRMP_Dialog):
    ...
    def _setupDialog(self):
        ...
        contact = self.addWidget(PRMP_LabelFrame, config=dict(text='Contact Details'), place=dict(relx=0, rely=0, h=200, relw=.55))
        
        self.name = LabelEntry(contact,  topKwargs=dict(text='Name'), bottomKwargs=dict(placeholder='Love'), orient='h', relx=.02, rely=0, relh=.15, relw=.96, longent=.25)
        
        self.phone = LabelEntry(contact,  topKwargs=dict(text='Phone Number'), relx=.02, rely=.14, relh=.15, relw=.96, longent=.5, orient='h')
        
        self.email = LabelEntry(contact,  topKwargs=dict(text='Email'), bottomKwargs=dict(type_='email'), relx=.02, rely=.28, relh=.15, relw=.96, longent=.25, orient='h')
        
        self.gender = LabelCombo(contact,  topKwargs=dict(text='Gender'), bottomKwargs=dict(type_='gender'), relx=.02, rely=.42, relh=.15, relw=.96, longent=.25, orient='h')
        
        self.address = LabelText(contact,  topKwargs=dict(text='Address'), relx=.02, rely=.56, relh=.44, relw=.96, longent=.3, orient='h')
