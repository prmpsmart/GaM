...
class Thrift_Analysis:
 ...
 # class Thrift_Analysis(Frame):
    ...
    def __init__p(self, master, small_details, full_details, details_output, **kw):
      ...
      ############ Specific Datas
        ...
        self.s_d_month = RadioCombo(self.s_data_lblfrm, text='Months', variable=self.spec_cbtn, relx=.05, rely=.16, relh=.4, relw=.45, values=MONTHS_NAMES[1:], value='spec_month', command=self.spec_choosable, func=self.get_spec_month)

        self.s_d_week = RadioCombo(self.s_data_lblfrm, text='Weeks', variable=self.spec_cbtn, relx=.05, rely=.57, relh=.4, relw=.45, values=Weeks.weeks,  value='spec_week', command=self.spec_choosable, func=self.get_spec_week)

        self.s_d_day = RadioCombo(self.s_data_lblfrm, text='Days', variable=self.spec_cbtn, relx=.55, rely=.16, relh=.4, relw=.4, values=DAYS_NAMES,  value='spec_day', command=self.spec_choosable, func=self.get_spec_day)


        self.s_d_area = RadioCombo(self.s_data_lblfrm, text='Areas', variable=self.spec_cbtn,  relx=.55, rely=.57, relh=.4, relw=.4, bordermode='ignore',value='spec_area', command=self.spec_choosable, func=self.get_spec_area)
 ...
 ######### Choosable buttons
    ...
    def spec_choosable(self):
        ...
        self.s_d_week.checked()
        self.s_d_day.checked()
        self.s_d_area.checked()
