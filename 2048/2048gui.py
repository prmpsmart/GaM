...
class PRMP_Game(Tk, Game2048):
    ...
    #Creates buttons at top of screen
    def create_widgets(self):
        self.buttonframe = Frame(self.titleBar, place=dict(relx=0, rely=0, relh=.9, relw=1))
        Button(self.buttonframe, text = "New Game",command=self.new_game).grid(row=0, column=0)
        Label(self.buttonframe, text = "Score:").grid(row=0, column=1)
        Label(self.buttonframe, textvariable=self.scorestring).grid(row=0, column=2)
        Label(self.buttonframe, text = "Record:").grid(row=0, column=3)
        Label(self.buttonframe, textvariable=self.highscorestring).grid(row=0, column=4)
        self.buttonframe.paint()
        self.canvas = Canvas(self.container, config=dict(width=410, height=410, borderwidth=2, highlightthickness=0), highlightable=0)
        self.canvas.pack(side="top", fill="both", expand="false")
